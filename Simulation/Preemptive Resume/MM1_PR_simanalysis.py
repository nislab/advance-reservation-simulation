'''
MM1_PR_simanalysis - plots of results from MM1 PR Advanced Reservation simulations
'''
...
for i in range(1,10):
	...
	for j in range(9):
		...
		for k in range(BUCKETS):
			if ((k)/BUCKETS) < phi:
				w_ana[k] = 1/((1-lam)*(1-lam*(1-phi))) - 1
			else:
				w_ana[k] = 1/((1-lam*(1-k/BUCKETS))*(1-lam*(1-(k+1)/BUCKETS))) - 1
		'''
		#plot results
		'''
		x = np.zeros(BUCKETS)
