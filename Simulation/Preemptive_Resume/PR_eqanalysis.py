'''
MM1_PR_eqanalysis - plots to support the equilibrium analysis for MM1 AR with preemptive resume
'''
...
for l in range(3):
	...
	C = np.zeros(1001) 
	for p in range(1001):
		C[p] = (lam*PHI[p])/((1-lam)*(1-lam*(1-PHI[p]))**2)
	plt.plot(PHI, C, label='C($\\phi$) analytical') 
	C_under =  lam/(1-lam) # C underbar; defined as C(1)
	#plt.hlines(C_under, PHI[0], PHI[1000], colors='r', linestyles='dashed', label = '$\\underline{C}$')
	if l > 5:
		C_over = 1/(4*((1-lam)**2)) # C overbar; defined as maximum of function, if lambda/mu > 1/2
