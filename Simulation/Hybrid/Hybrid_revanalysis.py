'''
MM1_NP_eqanalysis - plots to support the equilibrium analysis for MM1 HPR AR queue
'''
...
for l in range(2):
	...
	R = np.zeros(1001) 
	for p in range(1001):
		R[p] = (2*lam*(1-lam)*(1-PHI[p])*(1-lam*(1-PHI[p]))+2*(lam**2)*PHI[p]*(1-PHI[p]))/(2*(1-lam)*(1-lam*(1-PHI[p]))**2) - lam*(1-PHI[p])
