"""
Module to define simulator for M|M|1 queues with Advanced Reservation, with hybrid preemption/non preemption
...
def Simulator(LAM, MU, PHI, revfile):
    ...
    C = (1/MU)*((2*(1-RHO)*(1-RHO*(1-PHI))+2*RHO*PHI)/(2*(1-RHO)*(1-RHO*(1-PHI))**2)-1) # Cost to make reservation  
    '''
    ...
    class PriorityQueue:
        ...
        def push(self, priority, entry, service, madeAR):
            heapq.heappush(self.items, (priority, entry, service, madeAR))
    ...
    class SimEnv:
        ...
        def arrivals(self, env):
            ...
            while True:
                ...
                self.q.push(-1*priority, env.now, service, madeAR) 
                ...
                elif madeAR and (not self.next[3]):
        ...
        def server(self, env):
            ...
            while True:
                ...
                try:
                    ...
                    self.q.push(0, self.next[1], self.next[2]-(env.now-start), False) # decrement time remaining by amount of time passed            
