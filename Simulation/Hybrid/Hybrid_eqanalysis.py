'''
MM1_HPR_eqanalysis - plots to support the equilibrium analysis for MM1 HPR AR queue
'''
...
for l in range(3):
	...
	C = np.zeros(1001) 
	for p in range(1001):
		C[p] = ((1-lam)*(1-lam*(1-PHI[p]))+lam*PHI[p])/((1-lam)*((1-lam*(1-PHI[p]))**2)) - 1
	plt.plot(PHI, C, label='C($\\phi$) analytical') 
	'''
	C_under =  (lam**2)/(2*(1-lam)) # C underbar; defined as C(1)
	plt.hlines(C_under, PHI[0], PHI[1000], colors='r', linestyles='dashed', label = '$\\underline{C}$')
	if l > 5:
		#C_over = lam/(8*((1-lam)**2)) # C overbar; defined as maximum of function, if lambda/mu > 1/2
