"""
Module to define simulator for M|M|1 queues with Advanced Reservation, but no preemption
...
def Simulator(LAM, MU, PHI, meanfile, errfile, normfile, numfile, costfile):
    ...
    class PriorityQueue:
        ...
        def push(self, priority, entry, isGhost, index):
            heapq.heappush(self.items, (priority, entry, isGhost, index))
    ...
    class SimEnv:
        ...
        def arrivals(self, env):
            ...
            while True:
                ...
                self.q.push(-1*priority,env.now, False, i) 
        ...
        def ghost(self, env):
            ...
            while True:
                ...
                # add arrival to queue, as tuple of (priority, arrival time, isGhost, index); priority is negated due to tuple sorting rules 
                self.q.push(-1*priority,env.now, True, i)
        ...
        def server(self, env):
            ...
            while True:
                ...
                # otherwise, is actual customer, run job for some exponential time units;
                # if beyond threshold record wait time in queue, add to total_w and increment number of customers seen in the arrival percentile; 
                else:
                    self.idle = False
                    if (env.now > T_START):
                        self.total_w[self.next[3]] += (env.now - self.next[1])
